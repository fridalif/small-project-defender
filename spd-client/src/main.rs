...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    if config.ssh_detector.check_auth_on {
        let config_clone = std::sync::Arc::clone(&config);
        let tx = tx.clone();
        let handle = thread::spawn(move || ssh_detector::ssh_auth_log_watcher(std::sync::Arc::clone(&config_clone), tx.clone()));
        thread_handles.push(handle);
    }
    if config.ssh_detector.check_journalctl_on {
        let config_clone = std::sync::Arc::clone(&config);
        let tx = tx.clone();
        let handle = thread::spawn(move || ssh_detector::journalctl_watcher(std::sync::Arc::clone(&config_clone), tx.clone()));
        thread_handles.push(handle);
    }
    if config.port_detector.on {
        let config_clone = std::sync::Arc::clone(&config);
        let tx = tx.clone();
        let handle = thread::spawn(move || port_detector::port_detector(std::sync::Arc::clone(&config_clone), tx.clone()));
        thread_handles.push(handle);
    }
    {
        //let origins_hash_scaner = std::sync::Arc::clone(&origins_hash_scaner);
        let config_clone = std::sync::Arc::clone(&config);
        let handle = thread::spawn(move || connection::serve(std::sync::Arc::clone(&config_clone), rx));
        thread_handles.push(handle);
    }
